...
import API from "../api";
import { Pencil, Trash2, ExternalLink, Github, Briefcase, Plus } from "lucide-react";
...
export default function Freelancing() {
  ...
  const fetchFreelance = async () => {
    try {
      ...
      setFreelanceList(res.data);
    } catch (err) {
      console.error("Error fetching freelancing data:", err);
    }
  };
  ...
  const deleteFreelance = async (id) => {
    try {
      await API.delete(`/freelancing/${id}`);
      fetchFreelance();
    } catch (err) {
      console.error("Error deleting freelancing record:", err);
    }
  };
